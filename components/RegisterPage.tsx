...
const RegisterPage: React.FC<RegisterPageProps> = ({ setView, setError, error, themeStyles, groups }) => {
  ...
  if (success) {
    return (
      <div className={`fixed inset-0 overflow-y-auto flex items-center justify-center p-4 transition-colors duration-500 ${themeStyles.bg}`}>
  ...
  // MAIN REGISTER VIEW
  // FIX: Menggunakan 'fixed inset-0 overflow-y-auto' agar halaman bisa discroll meskipun body locked
  return (
    <div className={`fixed inset-0 z-50 overflow-y-auto overflow-x-hidden ${themeStyles.bg} transition-colors duration-500`}>
      
      <div className="min-h-full w-full flex flex-col items-center justify-start md:justify-center p-4 md:p-8 relative">
        <BackgroundOrnament colorClass={themeStyles.bgPatternColor} />
        
        {/* Container adapts width based on step. */}
        <div className={`w-full ${step === 1 ? 'max-w-7xl' : 'max-w-xl'} ${themeStyles.card} rounded-3xl p-6 md:p-10 ${themeStyles.glow} relative z-10 transition-all duration-500 my-auto`}>
          
          {/* Header Title */}
          <div className="text-center mb-8">
            <h2 className={`text-2xl md:text-4xl ${themeStyles.fontDisplay} font-bold ${themeStyles.textPrimary} tracking-widest uppercase`}>
          ...
          {step === 1 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500 flex flex-col items-center">
               ...
               {/* Component ini sekarang aman discroll karena parentnya overflow-y-auto */}
               <AvatarSelection 
               ...
               <div className="mt-12 flex flex-col md:flex-row items-center justify-center w-full gap-4 pb-8 md:pb-0">
               ...
               <p className={`mt-6 text-center text-sm ${themeStyles.textSecondary} pb-4`}>
                 Sudah punya akun? <button onClick={() => setView('login')} className={`font-bold hover:underline ${themeStyles.textAccent}`}>Login</button>
               </p>
            </div>
          )}
          ...
          {/* STEP 2: FORM */}
          {step === 2 && (
            <form onSubmit={handleRegister} className="grid grid-cols-1 gap-5 animate-in fade-in slide-in-from-right-4 duration-500 pb-4">
